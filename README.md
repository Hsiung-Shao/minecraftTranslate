...
| 模型 | VRAM 需求 | 翻譯品質 | 建議量化 | 大小 |
|------|----------|---------|---------|------|
| **Qwen3 8B** | 6 GB | 好 | Q4_K_M | ≈ 5 GB |
| **Qwen3 14B** | 10 GB | 優秀 | Q5_K_M | ≈ 10 GB |
| **Qwen3 32B** | 20 GB | 最佳 | Q4_K_M | ≈ 19 GB |

**下載步驟：**
1. 左側列表找到想要的模型（例如 **Qwen3 8B**，不是 VL 版本）
2. 右側面板確認版本資訊（Params、Arch、Format 應為 `GGUF`）
3. **Download Options** 區選擇量化（Q4_K_M 為平衡選擇，Q5 品質更好但檔案更大）
4. 若看到 🟢 **Full GPU Offload Possible**，代表你的 VRAM 能完整載入
5. 點 **Download** 開始下載（視網速需數分鐘）

> 💡 Qwen3 系列的中文能力特別強，是翻譯 Minecraft MOD 的首選。若 VRAM 較小，選高量化等級的小模型（8B Q5 > 14B Q3）。
...
- **Enable Thinking**：若用 thinking 模型（如 Qwen3-Reasoning），可保持開啟；純翻譯建議關閉以加快速度
