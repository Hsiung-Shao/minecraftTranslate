...
| 整合包 | 模組數 | 字串數 | 耗時（Thinking 關） | 耗時（Thinking 開，預估） |
|--------|:-----:|:-----:|:-------------------:|:-------------------------:|
| Craft to Exile 2 (VR Support) | ~250 | ~33,000 | **≈ 1.5 小時** | 16-33 小時（1000-2000 分鐘） |
| All the Mons - ATMons | ~240 | ~40,000 | **≈ 1.5 小時** | 16-33 小時（1000-2000 分鐘） |

> ⚠️ **上表的 1.5 小時是「已關閉 Enable Thinking」的實測數字**。若忘記關閉 Thinking 模式，以相同硬體估算會飆升到 **16-33 小時**（整整一天以上）。這是使用 thinking 模型時最容易踩到的坑。
>
> ⚡ **速度關鍵**（依重要性排序）：
> 1. **關閉 Enable Thinking** — 影響最大，快 10-20 倍
> 2. **加大 Context 長度與 batch size** — 減少 API 呼叫次數
> 3. **啟用並行 workers** — 讓 LM Studio 同時處理多個請求
